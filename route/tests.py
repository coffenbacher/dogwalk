...
class RouteFindingTest(TestCase):
    ...
    def test_solver_basic(self):
        s = Node.objects.get(id=1)
        v = Node.objects.filter(id=2)
        e = Node.objects.get(id=3)
        p = Problem.objects.create(start=s, end=e)
        p.save()
        p.visits = v
        route = p.solve()
        self.failUnlessEqual(list(route), [Node.objects.get(pk=1),Node.objects.get(pk=2),Node.objects.get(pk=3)])
    
    def test_solver_basic4(self):
        s = Node.objects.get(id=1)
        v = Node.objects.filter(id__in=(3,2)).order_by('-pk')
        e = Node.objects.get(id=4)
        p = Problem.objects.create(start=s, end=e)
        p.save()
        p.visits = v
        route = p.solve()
        self.failUnlessEqual(list(route), [Node.objects.get(pk=1),Node.objects.get(pk=2),Node.objects.get(pk=3), Node.objects.get(pk=4)])
